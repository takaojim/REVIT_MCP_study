...
## [L-032] 建築設計模型圖元量級與 MCP 查詢上限（maxCount 10,000）原則
...
## [L-033] 標註型式 (DimensionType) 前置動態查詢與降級防呆原則
...
## [L-034] 立面圖/剖面圖雙層標註標準工作流與 Revit 2026 (.NET 8) 建置部署原則
...
## [L-035] 平面圖實體外框抓取、2D 軸線嚴格共線與配置 A 規範
...
## [L-036] 柱心尺寸標註型式強制綁定與 Transaction 原生切換機制
...
## [L-037] 建築平面圖四向三層牆心標註體系、15CM主牆過濾與中庭內凹方案 C 規範
...
## [L-038] 建築立面與剖面 2D 計算幾何 Silhouette 外輪廓提取 (Clipper2)、GL 基準釘死、Silhouette 絕對錨定與左側 N+3 階梯整列標準工作流
...
## [L-039] 平面與立面尺寸標註多尺度自適應模矩換算體系 (Scale-Aware Formula) 與牆心正交量測原則
