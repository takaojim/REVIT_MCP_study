...
async function main() {
  ...
  client.clientName = 'batch-align-8steps';
  await client.connect();

  console.log('================================================================');
  console.log('=== 【全樓板平面圖】實體外框抓取與 8 個間距軸線氣泡齊頭整列 ===');
  console.log('================================================================\n');

  // 定義要處理的樓板平面圖視圖清單 (包含 RFL 與 TRFL 繼承 5FL 基準)
  const targetViews = [
  ...
  console.log(`\n================================================================`);
  console.log(`=== 🎉 全樓板平面圖 8 間距齊頭整列與輔助線更新完成總覽 ===`);
  console.log(`================================================================`);
  for (const s of summary) {
    console.log(`- 視圖: ${s.name.padEnd(8)} | 軸線: ${s.grids} 條 | 基準: ${s.ref.padEnd(6)} | 齊頭 Top: ${s.bounds.TopY.toFixed(1)} mm | 狀態: ${s.status}`);
  }
