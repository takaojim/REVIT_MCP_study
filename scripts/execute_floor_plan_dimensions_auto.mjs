...
async function main() {
  ...
  client.clientName = 'floor-plan-auto-runner';
  ...
  for (const dt of dimTypes) {
    if (dt.DimensionTypeName.includes('柱心-上右') || dt.DimensionTypeName.includes('上右')) {
      typeIdUpRight = dt.DimensionTypeId;
    }
    if (dt.DimensionTypeName.includes('柱心-下右') || dt.DimensionTypeName.includes('下右')) {
      typeIdDownRight = dt.DimensionTypeId;
    }
  }

  // 若無上右/下右，使用標準 TABC-DIM 或預設線性標註
  if (!typeIdUpRight) {
  ...
  console.log(`標註型式配置:`);
  console.log(`- 北/東側型式 ID: ${typeIdUpRight}`);
  console.log(`- 南/西側型式 ID: ${typeIdDownRight}\n`);
  ...
  for (const fv of floorPlanViews) {
    ...
    if (northGrids.length >= 2) {
      ...
      if (resT1.success && resT1.data?.DimensionId && typeIdUpRight) {
      ...
      if (resT2.success && resT2.data?.DimensionId && typeIdUpRight) {
        dimsToChangeType.push({ id: resT2.data.DimensionId, typeId: typeIdUpRight });
      }
      console.log(`  ✓ 北側雙層柱心標註完成`);
    }
    ...
    if (southGrids.length >= 2) {
      ...
      if (resT1.success && resT1.data?.DimensionId && typeIdDownRight) {
      ...
      if (resT2.success && resT2.data?.DimensionId && typeIdDownRight) {
        dimsToChangeType.push({ id: resT2.data.DimensionId, typeId: typeIdDownRight });
      }
      console.log(`  ✓ 南側雙層柱心標註完成`);
    }
