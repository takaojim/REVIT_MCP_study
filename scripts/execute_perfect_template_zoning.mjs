...
// 公版雙欄幾何座標規範 (完全貼合出圖圖框)
// 左欄 Table 1: X = 2113.99 -> 2507.07 mm (W = 393.08 mm)
// 右欄 Table 2: X = 2509.38 -> 2901.89 mm (W = 392.51 mm)
// 頂端 Y: 3494.25 mm, 底端 Y: 2973.58 mm (H = 520.67 mm)
const BOX_TOP_Y    = 3494.25;
const BOX_BOTTOM_Y = 2973.58;

const T1_X = {
  start: 2113.99,
  col1:  2113.99, // 15mm: 2113.99 -> 2128.99
  col2:  2128.99, // 220mm: 2128.99 -> 2348.99
  col3:  2348.99, // 158mm: 2348.99 -> 2507.07
  end:   2507.07
};

const T2_X = {
  start: 2509.38,
  col1:  2509.38, // 15mm: 2509.38 -> 2524.38
  col2:  2524.38, // 220mm: 2524.38 -> 2744.38
  col3:  2744.38, // 157.5mm: 2744.38 -> 2901.89
  end:   2901.89
};
...
// 完整 11 條法規全文資料庫（100% Verbatim，含 3 大子表格）
export const COMPLETE_ZONING_DATA = {
  leftColumnArticles: [
    {
      num: '一、',
    ...
    },
    {
      ...
      table2: {
        ...
        rows: [
          ...
          ['學校用地(文中、文小)', '50', '150', '-'],
          ['學校用地(文高)', '50', '200', '-'],
          ['市場用地(豐交-市3、豐交-市5-1、豐交-市5-2)', '60', '240', '-'],
          ['市場用地(潭-市1、潭-市2、潭-市3、潭-市4、潭-市5、潭-市6)', '80', '240', '-'],
          ['停車場用地(平面)', '10', '20', '-'],
          ['停車場用地(立體)', '80', '800', '-'],
          ['停車場用地(潭-停4)', '80', '400', '-'],
          ['自來水事業用地(豐交-自)', '50', '250', '-'],
...
1.本計畫區建築物附設停車空間設置標準依建築技術規則設計施工編第59條所列第一類建築物用途，樓地板面積150平方公尺以下免設汽車停車位，但至少須設置1輛機車或自行車停車位，超過部分每100平方公尺設置1輛汽車與1輛機車或自行車停車位，其餘數部分超過50平方公尺應設置1輛汽車與1輛機車或自行車停車位；第二類建築物用途，樓地板面積150平方公尺以下免設汽車停車位，超過部分每150平方公尺設置1輛汽車停車位，其餘數部分超過75平方公尺應設置1輛汽車停車位，機車或自行車停車位以每戶設置1輛為原則。`,
      // 第 3 張圖：停車設置標準表
      parkingTable: {
        colWidths: [44.0, 56.0, 57.0, 57.0], // 總寬 214.0 mm (內縮 3mm)
        headersRow1: ['分區或用地', '總樓地板面積 (㎡)', '停車設置標準'],
        headersRow2: ['汽車', '機車(或自行車)'],
        rows: [
          { group: '第一類\n建築物', area: '1~200', car: '免設置', moto: '1' },
          { group: '', area: '超過200~300', car: '設1輛', moto: '2' },
          { group: '', area: '超過300~400', car: '設2輛', moto: '3' },
          { group: '', area: '以下類推', car: '--', moto: '--' },
          { group: '第二類\n建築物', area: '1~225', car: '免設置', moto: '每戶設1輛為原則' },
          { group: '', area: '超過225~375', car: '設1輛', moto: '' },
          { group: '', area: '超過375~525', car: '設2輛', moto: '' },
          { group: '', area: '以下類推', car: '--', moto: '' }
        ]
      },
      contentAfter: `2.依前述規定計算建築物新建、增建及既有建築物應留設之汽車停車位在5輛以下無法設置者，得繳納代金，並依「臺中市建築物附建防空避難設備或停車空間繳納代金及管理使用自治條例」辦理。
...
// 渲染單一固定公版欄位
function renderColumnTable(boxCoords, articles) {
  const { start, col1, col2, col3, end } = boxCoords;
  const lines = [];
  const notes = [];

  const LINE_PITCH = 5.4;     // 精算行距 (5.4mm)
  const TOP_PADDING = 3.0;    // mm
  const BOTTOM_PADDING = 4.0; // mm

  let curY = BOX_TOP_Y;

  // 1. 公版固定表頭 (頂線 + 大標題 + 欄名列 + 底線)
  ...
  for (const art of articles) {
    ...
    if (art.isIntensityTable) {
      ...
      let subVX = t1StartX;
      lines.push({ startX: subVX, startY: t1TopY, endX: subVX, endY: t1BottomY }); // 藍線 (左)
      for (let c = 0; c < subColWidths.length - 1; c++) {
        subVX += subColWidths[c];
        lines.push({ startX: subVX, startY: t1TopY, endX: subVX, endY: t1BottomY }); // 紅線 (內部)
      }
      lines.push({ startX: t1EndX, startY: t1TopY, endX: t1EndX, endY: t1BottomY });   // 藍線 (右)
      ...
      // 繪製停車表縱向格線
      let pvx = ptStartX;
      lines.push({ startX: pvx, startY: ptTopY, endX: pvx, endY: ptBottomY }); // 藍線 (左)
      pvx += ptColWidths[0];
      lines.push({ startX: pvx, startY: ptTopY, endX: pvx, endY: ptBottomY }); // 紅線 1
      pvx += ptColWidths[1];
      lines.push({ startX: pvx, startY: ptTopY, endX: pvx, endY: ptBottomY }); // 紅線 2
      pvx += ptColWidths[2];
      lines.push({ startX: pvx, startY: subHeaderY + 1.0, endX: pvx, endY: ptBottomY }); // 紅線 3
      lines.push({ startX: ptEndX, startY: ptTopY, endX: ptEndX, endY: ptBottomY }); // 藍線 (右)
  ...
  // 3. 公版固定四邊外框與 3 欄分界線 (嚴格繪製至 BOX_BOTTOM_Y)
  const vLineXPositions = [col1, col2, col3, end];
  for (const vx of vLineXPositions) {
    lines.push({ startX: vx, startY: BOX_TOP_Y, endX: vx, endY: BOX_BOTTOM_Y });
  }
  // 底線
  lines.push({ startX: start, startY: BOX_BOTTOM_Y, endX: end, endY: BOX_BOTTOM_Y });
...
async function main() {
  ...
  client.clientName = 'execute-perfect-template-zoning-' + Date.now();
  await client.connect();

  const viewId = 711441; // "2_都市計畫1 複製 1"
  console.log(`\n=== 開始在測試視圖【2_都市計畫1 複製 1】(ID: ${viewId}) 執行公版雙欄精確排版 ===\n`);

  // 1. 清理舊元素
  console.log('--- Step 1: 清理測試視圖既有元素 ---');
  ...
  // 2. 渲染公版左欄與右欄
  console.log('--- Step 2: 繪製固定公版外框、子表格線與條文文字 ---');
  const leftRes = renderColumnTable(T1_X, COMPLETE_ZONING_DATA.leftColumnArticles);
  const rightRes = renderColumnTable(T2_X, COMPLETE_ZONING_DATA.rightColumnArticles);

  const allLines = [...leftRes.lines, ...rightRes.lines];
  const allNotes = [...leftRes.notes, ...rightRes.notes];

  console.log(`發送 ${allLines.length} 條公版與子表格線條至 Revit...`);
  ...
  console.log('\n========================================================');
  console.log('✅ 【2_都市計畫1 複製 1】公版雙欄精確排版執行完成！');
  console.log('- 公版外框：高度嚴格鎖定於 520.67mm (Y: 3494.25 -> 2973.58)，邊界不動');
  console.log('- 三大表格：完整繪製 使用分區管制表、公共設施管制表、停車設置標準表');
  console.log('- 表格線條：左右兩側各內縮 3mm (藍線)，欄間縱向直線完整劃出 (紅線)');
  console.log('- 法規文字：100% 原文保留，全數統一套用【3 mm 微軟正黑體】');
  console.log('========================================================\n');
  process.exit(0);
}

if (process.argv[1] && process.argv[1].endsWith('execute_perfect_template_zoning.mjs')) {
  main().catch(err => {
