...
async function main() {
  ...
  client.clientName = 'floor-plans-7steps-corrected-' + Date.now();
  await client.connect();

  console.log('================================================================');
  console.log('=== 【樓板平面所有視圖】7 間距標準修正放樣 ===');
  console.log('=== (氣泡到柱心空 2 格 ＋ 柱心 2 層 ＋ 牆心 2 層 ＋ 外牆空 2 格) ===');
  console.log('================================================================\n');
  ...
  console.log(`✓ 實體外框 (紅線基準, Step 0): X=[${globalEnv.MinX.toFixed(1)}, ${globalEnv.MaxX.toFixed(1)}], Y=[${globalEnv.MinY.toFixed(1)}, ${globalEnv.MaxY.toFixed(1)}]`);
  console.log(`✓ 7 間距齊頭線 (藍線基準, Step 7, +4550mm): Top=${globalBounds7.TopY.toFixed(1)}, Right=${globalBounds7.RightX.toFixed(1)} mm\n`);
  ...
  // Step 7: 藍線 (4550mm)
  // Step 6: 留白 1 (3900mm)
  // Step 5: 柱心總跨 (3250mm) -> 距藍線精確空 2 格 (1300mm)
  // Step 4: 柱心連續 (2600mm) -> 距總跨 1 格 (650mm)
  // Step 3: 牆心 Layer 1 外牆總長 (1950mm) -> 距柱心 1 格 (650mm)
  // Step 2: 牆心 Layer 2 居室隔間 (1300mm) -> 距牆心1 1 格 (650mm)
  // Step 1: 留白 (650mm) -> 牆心2 距外牆紅線空 2 格 (1300mm)，完美防遮蔽！
  // Step 0: 紅線外牆面 (0mm)

  const northColTier1Y = globalEnv.MaxY + 3250.0;
  const northColTier2Y = globalEnv.MaxY + 2600.0;
  const eastColTier1X = globalEnv.MaxX + 3250.0;
  const eastColTier2X = globalEnv.MaxX + 2600.0;

  const northWallStep1Y = globalEnv.MaxY + 1950.0; // 牆心 Layer 1 (外牆總長)
  const northWallStep2Y = globalEnv.MaxY + 1300.0; // 牆心 Layer 2 (居室隔間)
  ...
  // 輔助函式：建立牆心標註
  async function createWallDim(viewId, sideName, layerNum, desc, walls, isVerticalAxis, dimCoord, isAscending) {
    ...
    if (res.success && res.data?.DimensionId) {
      ...
      console.log(`    ✓ [${sideName} 牆心 Layer ${layerNum} (${desc})] ID: ${res.data.DimensionId}, 分段: ${unique.length - 1}`);
  ...
  for (const v of floorPlanViews) {
    console.log(`\n----------------------------------------------------------------`);
    console.log(`🚀 [${v.name}] (View ID: ${v.id}) 執行修正放樣 (氣泡空2格 ＋ 柱心2層 ＋ 牆心2層 ＋ 外牆空2格)...`);
    ...
    // (D) 北側與東側 柱心標註 (Step 5 總跨 3250mm, Step 4 連續柱心 2600mm)
    ...
    console.log(`  ✓ 柱心雙層標註放樣完成 (Step 5 總跨: 3250mm, Step 4 連續: 2600mm)`);
    ...
    // (F) 建立 4 向 2 層牆心標註 (Step 3: 外牆總長 1950mm, Step 2: 居室主隔間 1300mm)
    // 東側
    await createWallDim(v.id, '東側', 1, '外牆總長', horizWalls.filter(w => w.maxX > globalEnv.MaxX - 5000), true, eastWallStep1X, true);
    await createWallDim(v.id, '東側', 2, '居室主隔間', horizWalls.filter(w => w.maxX > globalEnv.MaxX - 8000), true, eastWallStep2X, true);

    // 西側
    await createWallDim(v.id, '西側', 1, '外牆總長', horizWalls.filter(w => w.minX < globalEnv.MinX + 5000), true, westWallStep1X, true);
    await createWallDim(v.id, '西側', 2, '居室主隔間', horizWalls.filter(w => w.minX < globalEnv.MinX + 8000), true, westWallStep2X, true);

    // 北側
    await createWallDim(v.id, '北側', 1, '外牆總長', vertWalls.filter(w => w.maxY > globalEnv.MaxY - 5000), false, northWallStep1Y, false);
    await createWallDim(v.id, '北側', 2, '居室主隔間', vertWalls.filter(w => w.maxY > globalEnv.MaxY - 8000), false, northWallStep2Y, false);

    // 南側
    await createWallDim(v.id, '南側', 1, '外牆總長', vertWalls.filter(w => w.minY < globalEnv.MinY + 5000), false, southWallStep1Y, false);
    await createWallDim(v.id, '南側', 2, '居室主隔間', vertWalls.filter(w => w.minY < globalEnv.MinY + 8000), false, southWallStep2Y, false);

    summary.push({
      name: v.name,
      id: v.id,
      columnDims: 'Step 5 & Step 4 (距氣泡空2格)',
      wallDims: 'Step 3 & Step 2 (距外牆空2格)',
      status: 'SUCCESS'
    });
  }

  console.log('\n================================================================');
  console.log('=== 【樓板平面所有視圖】7 間距標準修正放樣全數完成！ ===');
  console.log('================================================================');
