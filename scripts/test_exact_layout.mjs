...
// 絕對防禦字寬權重：40.0 (每行最多約 38~40 個全形字，Revit 絕對 0% 機會二次折行！)
const EXACT_CLAUSE_WEIGHT = 40.0;
...
function calcClause(text) {
  const wrapped = wrapText(text, EXACT_CLAUSE_WEIGHT);
  const lineCount = wrapped.split('\n').length;
  // 頂部留白 2.0mm，底部留白 2.5mm，總高 = N * 5.8 + 4.5mm
  return { wrapped, lineCount, height: lineCount * LINE_PITCH + 4.5 };
}

function simulateExactLayout() {
  console.log('=== 精算 40.0 權重單頁黃金方案 ===\n');
  const maxAvailableH = BOX_TOP_Y - BOX_BOTTOM_Y - 20.0; // 500.67mm

  // 左欄
  let leftH = 0;
  leftH += calcClause('本要點依都市計畫法第22條及同法臺灣省施行細則第35條規定訂定之。').height; // 第1點
  leftH += calcClause('本計畫各種土地使用分區之建蔽率及容積率規定詳如下表：').height; // 第2點前言
  leftH += (7.0 + 7 * 6.5 + 2.0); // 第2點分區表
  leftH += calcClause('註：變更和美主要計畫（第四次通盤檢討）案後續報請核定案件第6案原站2之容積率上限，其他住宅區為200%。').height;
  leftH += calcClause('本計畫各種公共設施用地之建蔽率及容積率規定詳如下表：').height; // 第3點前言
  leftH += (7.0 + 9 * 6.0 + 12.0 + 2.0); // 第3點公設表
  leftH += calcClause('古蹟保存區以供保存維護古物、古蹟及有關文物、文化景觀之使用為限，其土地及建築物依現況為準，不得增建及新建。').height; // 第4點
...
（三）其他依郵政法第5條規定及經濟部核准中華郵政公司可營利事業項目之服務項目前提下，除經直轄市、縣（市）政府審查核准之必要設施外，不得作為商業使用。`;
  const c6 = calcClause(text6);
  leftH += c6.height;

  leftH += calcClause('農業區應依都市計畫法台灣省施行細則第29條規定辦理，不得設置營建剩餘土石方資源堆置場及廢棄物資源回收貯存場。').height;

  console.log(`第5點精算行數: ${c5.lineCount} 行, 高度: ${c5.height.toFixed(2)} mm`);
  console.log(`第6點精算行數: ${c6.lineCount} 行, 高度: ${c6.height.toFixed(2)} mm`);
  console.log(`【左欄總高】: ${leftH.toFixed(2)} mm / ${maxAvailableH.toFixed(2)} mm (剩餘: ${(maxAvailableH - leftH).toFixed(2)} mm)\n`);

  // 右欄
  let rightH = 0;
  rightH += calcClause('本計畫建築退縮規定詳如下表，其退縮部分得計入法定空地：').height;
  rightH += 158.2 + 2.0; // 退縮表
  rightH += calcClause('前項表內所示，建築基地臨接2條計畫道路者，倘無其他特殊規定，則其臨計畫道路部分皆應退縮。').height;
  rightH += calcClause('住宅區、商業區之建築基地於申請建築時，為考量都市發展，訂定停車空間設置標準如下。但基地情形特殊經提縣都市設計審議委員會審議同意者，從其規定。').height;
  rightH += calcClause('1.實施區段徵收或市地重劃地區及1,000平方公尺以上基地由低使用強度變更為高使用強度之整體開發地區，其停車空間應依下表規定辦理。').height;
  rightH += 31.0 + 2.0; // 停車表
  rightH += calcClause('2.前款以外地區，依「建築技術規則」規定辦理。').height;
...
8.綠園道用地之綠化面積不得低於20%。`;
  const c10 = calcClause(text10);
  rightH += c10.height;

  const c11 = calcClause('本計畫區之公共設施及公共建築應配合整體規劃設計，且符合「建築技術規則建築設計施工編」綠建築基準有關建築基地綠化、建築基地保水、建築物節約能源、建築物雨水或生活雜排水回收再利用、綠建材等規定。');
  rightH += c11.height;
...
2.建築物留設空間與天橋或地下道連接供公眾使用，經交通主管機關核准者。`;
  const c13 = calcClause(text13);
  rightH += c13.height;

  rightH += calcClause('本計畫區各建築基地除依都市更新法規實施都市更新事業之地區外，依本要點及其他相關法令規定所給予之獎勵容積，以不超過基地面積乘以該基地容積率之20％為限。').height;
  rightH += calcClause('本要點未規定之事項，適用其他法令規定。').height;

  console.log(`第10點精算行數: ${c10.lineCount} 行, 高度: ${c10.height.toFixed(2)} mm`);
  console.log(`第12點精算行數: ${c12.lineCount} 行, 高度: ${c12.height.toFixed(2)} mm`);
  console.log(`第13點精算行數: ${c13.lineCount} 行, 高度: ${c13.height.toFixed(2)} mm`);
