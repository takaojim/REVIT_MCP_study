...
async function main() {
  ...
  client.clientName = 'rfl-original-dimension-runner';
  await client.connect();

  console.log('=== 回到原柱間距標註技能：執行 RFL 平面柱心雙層標註 ===\n');
  ...
  console.log(`📌 作用中平面視圖: "${activeView?.Name}" (ID: ${viewId}, 比例 1:${scale})\n`);
  ...
  // 5. 依據第2張圖所定義的標準位置（精準階梯退縮 650 mm）
  const realBubbleLeftX = -2547.2;
  const realBubbleRightX = 20190.8;
  const realBubbleTopY = 5013.6;
  const realBubbleBotY = -18384.4;
  const step65 = 650.0;
  ...
  if (vertGrids.length >= 2) {
    const lineY_T1 = realBubbleTopY - step65;        // 4363.6 mm (總尺寸)
    const lineY_T2 = realBubbleTopY - (step65 * 2);  // 3713.6 mm (柱間距)
    ...
    console.log(`  ✓ 北側頂部柱心雙層標註 (Y_T1=${lineY_T1.toFixed(1)}, Y_T2=${lineY_T2.toFixed(1)})`);
  }

  // --- 南側 (下方) ---
  if (vertGrids.length >= 2) {
    const lineY_T1 = realBubbleBotY + step65;        // -17734.4 mm (總尺寸)
    const lineY_T2 = realBubbleBotY + (step65 * 2);  // -17084.4 mm (柱間距)
    ...
    console.log(`  ✓ 南側底部柱心雙層標註 (Y_T1=${lineY_T1.toFixed(1)}, Y_T2=${lineY_T2.toFixed(1)})`);
  }

  // --- 西側 (左側) ---
  if (horizGrids.length >= 2) {
    const lineX_T1 = realBubbleLeftX + step65;        // -1897.2 mm (總尺寸)
    const lineX_T2 = realBubbleLeftX + (step65 * 2);  // -1247.2 mm (柱間距)
    ...
    console.log(`  ✓ 西側左側柱心雙層標註 (X_T1=${lineX_T1.toFixed(1)}, X_T2=${lineX_T2.toFixed(1)})`);
  }

  // --- 東側 (右側) ---
  if (horizGrids.length >= 2) {
    const lineX_T1 = realBubbleRightX - step65;        // 19540.8 mm (總尺寸)
    const lineX_T2 = realBubbleRightX - (step65 * 2);  // 18890.8 mm (柱間距)
    ...
    console.log(`  ✓ 東側右側柱心雙層標註 (X_T1=${lineX_T1.toFixed(1)}, X_T2=${lineX_T2.toFixed(1)})`);
  }
  ...
  console.log(`\n✨ 視圖【${activeView?.Name}】柱心雙層標註已完全依據第2張圖標準就位！`);
