...
async function main() {
  ...
  client.clientName = 'inspect-current-project';
  await client.connect();

  console.log('=== 查詢當前開啟專案之所有樓板平面圖視圖 ===\n');
  ...
  for (const fp of floorPlans) {
    console.log(`- ID: ${fp.id.toString().padEnd(8)} | 名稱: "${fp.name.padEnd(20)}" | 類型: "${fp.typeName}" | 比例: 1:${fp.scale}`);
  }
